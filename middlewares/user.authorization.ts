...
export const authorize = (softPass = false) => {
  return async (req: any, res: any, next: any) => {
    try {
      if (req.headers["authorization"] === undefined && softPass === false) { // Checking for missing authorization token
        throw {
        ...
        next();
      } else { // If an authorization token exists, verifying it
        try {
          let decoded: any = "";
          const authorizationHeaderArray =
            req.headers["authorization"].split(" ");
          if (authorizationHeaderArray[0] !== "Bearer") { // Checking for token type
            throw {
              statusObj: BAD_REQUEST,
          ...
          decoded = jwt.verify( // Verifying the token using jwt.verify method
            authorizationHeaderArray[1],
            String(process.env.JWT_SECRET),
          );
          const user: any = await userService.findUserService({ // Fetching user details using the email id in the token
            email: decoded.email,
          });
          const urlsToSkip: Array<string> = [ // Creating a list of URLs that don't require token verification
            "/sendmail",
            "/verifytoken",
            "/profile",
            "/update",
          ...
          if (user.verified === false && urlsToSkip.includes(req.originalUrl)) { // Checking user verification status and original URL
            throw {
